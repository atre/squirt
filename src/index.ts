...
async function main(): Promise<void> {
  ...
  if (command === 'guard-stats') {
    const home = process.env.TALLY_HOME || join(homedir(), '.tally');
    const logPath = join(home, 'guard.log');
    if (!existsSync(logPath)) {
      console.log(`no guard log at ${logPath} — is tally installed? (tally hooks --install --global)`);
      return;
    }
    const text = await readFile(logPath, 'utf8');
    const since = flags.since ?? '7d';
    const n = guardStats(text, Date.now() - parseDuration(since));
    console.log(`${n.total} log dumps prevented (${since}) — ${n.rewritten} rewritten, ${n.blocked} blocked — via tally pre-bash`);
    return;
  }
  ...
  const print = (result: ClusterResult): void => {
    if (flags.show) {
      ...
      if (!sig) {
        console.error(`squirt: no signature with id ${JSON.stringify(flags.show)} in this input — ids are ephemeral: --show re-scans the input, so re-run the same pipeline with --show appended (e.g. \`… | squirt --show ${flags.show}\`)`);
        process.exitCode = 1;
        return;
      }
