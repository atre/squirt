...
// ── feedback round 2026-08-23: --causes ───────────────────────────────

test('--causes: parsed, clamped to 1..20, default 1', () => {
  assert.equal((parseArgs(argv('--causes', '3')).flags as any).causes, 3);
  assert.equal((parseArgs(argv('--causes', '99')).flags as any).causes, 20);
  assert.equal((parseArgs(argv()).flags as any).causes, 1);
});

test('--causes collects up to n cause lines per signature, incl. [cause]:', async () => {
  const r = await cluster(
    [
      'ERROR request failed',
      '    at handle (/app/x.js:1:1)',
      '    Caused by: FetchError: socket hang up',
      '    [cause]: TypeError: fetch failed',
    ],
    { causes: 3 } as any,
  );
  assert.deepEqual((r.signatures[0] as any).causes, [
    'Caused by: FetchError: socket hang up',
    '[cause]: TypeError: fetch failed',
  ]);
  assert.equal(r.signatures[0].detail, 'Caused by: FetchError: socket hang up');
  const text = renderText(r, { top: 20 });
  assert.equal((text.match(/⤷/g) ?? []).length, 2);
});

test('default cap 1: a [cause]: line now surfaces as detail, single ⤷', async () => {
  const r = await cluster(['ERROR boom', '    [cause]: TypeError: fetch failed']);
  assert.equal(r.signatures[0].detail, '[cause]: TypeError: fetch failed');
  const text = renderText(r, { top: 20 });
  assert.equal((text.match(/⤷/g) ?? []).length, 1);
});

test('--causes: JSON carries causes only when more than one was kept', async () => {
  const many = await cluster(
    ['ERROR boom', '    Caused by: IOError: x', '    [cause]: TypeError: y'],
    { causes: 3 } as any,
  );
  const j = JSON.parse(renderJson(many, { top: 20 }));
  assert.deepEqual(j.signatures[0].causes, ['Caused by: IOError: x', '[cause]: TypeError: y']);
  const one = await cluster(['ERROR boom', '    Caused by: IOError: x']);
  const j1 = JSON.parse(renderJson(one, { top: 20 }));
  assert.equal('causes' in j1.signatures[0], false);
});
