...
- [ ] **One log guard — hand rewriting to tally** — unwire `~/.claude/hooks/squirt-guard.sh` from `~/.claude/settings.json` (tally `hook pre-bash` already rewrites `kubectl logs|docker logs|journalctl|tail -n 300|cat *.log` → `| squirt` and has the fired data); `squirt init --claude` prints "global log guard is owned by `tally hooks --install`" and only ships the per-repo skill; `guard-stats` reads `~/.tally/guard.log` (or is removed). why: two rewriters per Bash call, one never fires · accept: after `tally hooks --install --global`, `tally hooks --list --global` shows one Bash rewriter; `squirt init --claude --global` makes no settings change. `dup → merge` · 2026-08-18: confirmed still duplicated (`tally hooks --list --global` shows both entries firing), `~/.squirt/guard.log` at 1 line total in 30d. A 2026-08-18 attempt to unwire it directly hit the harness permission classifier blocking agent edits to hook config — this item needs a human to run it, not an agent.
- [x] **Skip/downgrade dedup for non-log files** — what: before clustering, if the input source is a file (not stdin/kubectl/docker/journal) with an extension outside a small allowlist (`.log`, `.txt`, no extension), either skip signature-dedup (print near-verbatim) or tag the digest with a low-confidence warning; also narrow `squirt-guard.sh`'s rewrite targets so `tail`/`cat`/`grep` against `*.md`/`*.json`/`*.csv` aren't auto-piped through squirt in the first place. why: 2026-08-18 real hit — `tail -100 STATE.md` (10.6k-line markdown scorecard, not a log) got auto-rewritten through squirt; dedup collapsed the scorecard's numbered rows into `<n>`-placeholder buckets, destroying exactly the number being looked up (530) — squirt's near-dup heuristic assumes lines differ only in incidental fields (timestamps/ids), but in a table the number *is* the payload; → files: `src/cluster.ts` (extension/shape check before grouping — e.g. bail to a "not log-shaped, pass through" mode when the file extension is `.md`/`.json`/`.csv`), `src/init.ts` (`GUARD_SCRIPT`: exclude those extensions from the rewrite match), `test/cluster.test.ts` · accept: a markdown-table fixture through `cluster()` either skips masking the numeric column or the digest header flags low confidence; `squirt-guard.sh` given `tail -100 STATE.md` does not rewrite. `gap` · 2026-08-18: implemented as a content-based warning (not extension-based — the real incident arrived via piped stdin, so no filename was available); the squirt-guard.sh angle in the original write-up doesn't apply — squirt's own guard script only matches kubectl/docker/journalctl, never bare tail/cat, so it wasn't the rewriter that caused the incident (that was tally's separate pre-bash hook, out of scope for this repo).
- [x] **Push tag `v0.3.0`** so snuff can pin `github:atre/squirt#v0.3.0` (W.8 there). `hygiene` · 2026-08-18: already on origin (`v0.3.0` + `v0.3.1` both pushed) — snuff's own PLAN.md still says "local, pin after push", stale on their side.
- [x] **CLAUDE.md:35 "NO runtime npm dependencies"** contradicts line 9 → align with hub contract. `hygiene` · 2026-08-18: Stack line now states the same hard rule as Rules (deps-requiring work goes in a sibling package, e.g. the parked MCP wrapper).
