...
2. Interesting signature? `squirt … --grep '<re>' --sample 5` — more samples of just that one.
   Or grab its id from the digest (`#a3f1`) and `squirt … --show a3f1` to dump the raw lines behind it (same pipeline — ids aren't persisted between runs). Deep cause chain? `--causes 3` keeps up to 3 `⤷` cause lines per signature.
...
The global log-dump guard (rewrites raw `kubectl logs` / `docker logs` /
`journalctl` to `… | squirt`) is owned by tally: `tally hooks --install
--global`. `squirt init --claude` installs this skill into `.claude/skills/`
(`--global` for `~/.claude`). `squirt guard-stats` reads tally's guard log
and shows how many dumps were prevented.
