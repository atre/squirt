...
squirt init --claude                              # install the squirt skill into .claude/skills (the log guard itself is owned by `tally hooks --install`)
squirt guard-stats                                # N log dumps prevented (rewritten/blocked), last 7d — read from tally's guard.log
```
...
| `--causes <n>` | keep up to n folded cause lines per signature (default 1) |
...
The PreToolUse guard that rewrites raw `kubectl logs` / `docker logs` /
`journalctl` calls to `… | squirt` is owned by tally — install it once with
`tally hooks --install --global`. `squirt init --claude [--global] [--print]`
installs the squirt skill into `.claude/skills/squirt/` so agents reach for
squirt automatically. `squirt guard-stats [--since 7d]` reads tally's
`~/.tally/guard.log` (`<ts> <hook> <blocked|rewritten>`, `pre-bash` lines
only) and prints how many log dumps were prevented.
